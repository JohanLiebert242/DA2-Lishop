          ...
          <tbody>
            {adminInvoices.map((inv: AdminInvoice) => (
              <tr key={inv.id} className="border-b last:border-0 hover:bg-gray-50">
                ...
                <td className="px-4 py-3 font-mono text-sm text-gray-700">{inv.orderId.slice(0, 8)}…</td>
                <td className="px-4 py-3 text-sm text-gray-700">{inv.billingName}</td>
                <td className="px-4 py-3 text-sm font-medium text-gray-900">{formatVND(inv.totalVnd)}</td>
